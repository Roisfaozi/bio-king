  ...
  return (
    <section className='py-16 2xl:py-[120px]' id='whyDashTail'>
      <div className='container'>
        ...
        <div className='mt-[90px] grid grid-cols-1 gap-x-8 gap-y-16 md:grid-cols-2 xl:grid-cols-3'>
          {data.map((item, index) => (
            <div
              key={`about-dashtail-${index}`}
              className='relative rounded-sm border border-dotted border-default-300 p-6 pb-8 text-center hover:border-solid hover:border-primary'
            >
              <div className='absolute left-1/2 top-0 h-[72px] w-[72px] -translate-x-1/2 -translate-y-1/2'>
                <Image
                  src={item.icon}
                  alt={item.title}
              ...
              <div className=''>
                <h3 className='mb-3 pt-6 text-base font-semibold text-default-600 xl:text-xl'>
                  {item.title}
                </h3>
