...
const Workload = ({ height = 245 }: { height?: number }) => {
  ...
  return (
    <Card>
      <CardHeader className='mb-0 border-none p-6 pb-0 pt-7'>
        <CardTitle>Workload</CardTitle>
      </CardHeader>
      <CardContent className='mb-0 px-0 pb-1'>
      ...
      <CardFooter className='mt-0 items-center px-0'>
        {data.map((item, index) => (
          <div
            key={`workload-${index}`}
          ...
          >
            ...
            <div className='mt-2 hidden text-xs font-medium text-default-600 sm:block'>
              {item.name}
            </div>
          </div>
        ))}
